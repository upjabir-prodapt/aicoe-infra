...
resource "google_compute_router" "aicoe_router_cloudnat" {
  ...
  network = google_compute_network.aicoe_network[0].self_link
  region  = var.region
}
...
resource "google_compute_router_nat" "aicoe_cloudnat" {
  ...
  nat_ips = [
    google_compute_address.aicoe_staticip_nat_1[0].self_link,
    google_compute_address.aicoe_staticip_nat_2[0].self_link
  ]
