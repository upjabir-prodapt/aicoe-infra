...
resource "google_kms_key_ring" "aicoe_app_bucket_key_ring" {
  ...
  name     = "${var.resource_prefix}-app-bucket-key-ring"
  location = var.region
...
resource "google_kms_crypto_key" "aicoe_app_bucket_key" {
  count           = var.enable_kms ? 1 : 0
  name            = "${var.resource_prefix}-app-bucket-key"
  key_ring        = google_kms_key_ring.aicoe_app_bucket_key_ring[0].id
...
resource "google_kms_key_ring" "aicoe_vxai_wkb_key_ring" {
  ...
  name     = "${var.resource_prefix}-vxai-wkb-key-ring"
  location = var.region
  project  = var.gcp_project_id
}
...
resource "google_kms_crypto_key" "aicoe_vxai_wkb_key" {
  count           = var.enable_kms && var.enable_workbench_kms ? 1 : 0
  name            = "${var.resource_prefix}-vxai-wkb-key"
...
resource "google_storage_bucket" "buckets" {
  ...
  dynamic "encryption" {
    ...
    content {
      default_kms_key_name = google_kms_crypto_key.aicoe_app_bucket_key[0].id
    }
  }
...
resource "google_kms_crypto_key_iam_member" "app_sa_bucket_key" {
  ...
  crypto_key_id = google_kms_crypto_key.aicoe_app_bucket_key[0].id
